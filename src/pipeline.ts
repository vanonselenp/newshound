...
export function buildSummaryPrompt(rankedCandidates: RankedCandidate[], recentDigests: string[], digestConfig: DigestConfig): string {
  ...
  return `You are writing a daily ${digestConfig.name} for a practitioner who wants practical, actionable updates. Use fullText when present and snippet otherwise.\nRECENT DIGEST CONTEXT:\n${recent}${extraContext}\nChoose top unique stories from the ranked candidates below. Return ONLY a valid JSON object. Do not write markdown. Do not write frontmatter. Do not wrap the answer in code fences.\nThe JSON must have exactly these top-level keys: readInFull, highSignal, worthKnowing, tags, related.\nTags must come from: ${digestConfig.tags.join(', ')}\nJSON shape:\n{\n  "readInFull": [{"title": "...", "url": "...", "source": "...", "summary": "...", "takeaway": ""}],\n  "highSignal": [{"title": "...", "url": "...", "source": "...", "summary": "...", "takeaway": "..."}],\n  "worthKnowing": [{"title": "...", "url": "...", "source": "...", "summary": "..."}],\n  "tags": ["${digestConfig.tags[0] ?? 'tooling'}"],\n  "related": []\n}\n\n${items}`;
}
