...
describe('readRecentDigests', () => {
  ...
  it('returns empty array when vault directory does not exist', async () => {
    const result = await readRecentDigests('/nonexistent/path/vault', 7);
    expect(result).toEqual([]);
  });

  it('returns empty array when AI-Digest folder does not exist', async () => {
    const result = await readRecentDigests(vaultPath, 7);
    expect(result).toEqual([]);
  });
...
describe('writeDigest', () => {
  ...
  it('writes digest to correct path', async () => {
    ...
    const outputPath = await writeDigest(vaultPath, date, '# Test digest content');
  ...
  it('creates AI-Digest directory if it does not exist', async () => {
    const date = new Date('2026-03-30T08:00:00Z');
    const outputPath = await writeDigest(vaultPath, date, '# Content');
